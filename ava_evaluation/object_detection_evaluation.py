...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        groundtruth_classes = (
            groundtruth_dict[standard_fields.InputDataFields.groundtruth_classes] - self._label_id_offset
        )
        ...
        if standard_fields.InputDataFields.groundtruth_difficult in groundtruth_dict.keys() and (
            groundtruth_dict[standard_fields.InputDataFields.groundtruth_difficult].size or not groundtruth_classes.size
        ):
            groundtruth_difficult = groundtruth_dict[standard_fields.InputDataFields.groundtruth_difficult]
        else:
            groundtruth_difficult = None
            if not len(self._image_ids) % 1000:
                logging.warn("image %s does not have groundtruth difficult flag specified", image_id)
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        """
        detection_classes = (
            detections_dict[standard_fields.DetectionResultFields.detection_classes] - self._label_id_offset
        )
    ...
    def evaluate(self):
        ...
        pascal_metrics = {self._metric_prefix + f"Precision/mAP@{self._matching_iou_threshold}IOU": mean_ap}
        if self._evaluate_corlocs:
            pascal_metrics[
                self._metric_prefix + f"Precision/meanCorLoc@{self._matching_iou_threshold}IOU"
            ] = mean_corloc
        category_index = label_map_util.create_category_index(self._categories)
        for idx in range(per_class_ap.size):
            if idx + self._label_id_offset in category_index:
                display_name = self._metric_prefix + "PerformanceByCategory/AP@{}IOU/{}".format(
                    self._matching_iou_threshold, category_index[idx + self._label_id_offset]["name"]
                )
...
class OpenImagesDetectionEvaluator(ObjectDetectionEvaluator):
    ...
    def __init__(self, categories, matching_iou_threshold=0.5, evaluate_corlocs=False):
        ...
        """
        super().__init__(categories, matching_iou_threshold, evaluate_corlocs, metric_prefix="OpenImagesV2")
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        if standard_fields.InputDataFields.groundtruth_group_of in groundtruth_dict.keys() and (
            groundtruth_dict[standard_fields.InputDataFields.groundtruth_group_of].size or not groundtruth_classes.size
        ):
...
ObjectDetectionEvalMetrics = collections.namedtuple(
    "ObjectDetectionEvalMetrics", ["average_precisions", "mean_ap", "precisions", "recalls", "corlocs", "mean_corloc"]
)
...
class ObjectDetectionEvaluation:
    ...
    def __init__(
        ...
        self.per_image_eval = per_image_evaluation.PerImageEvaluation(
            num_groundtruth_classes=num_groundtruth_classes, matching_iou_threshold=matching_iou_threshold
        )
    ...
    def _update_ground_truth_statistics(
        self, groundtruth_class_labels, groundtruth_is_difficult_list, groundtruth_is_group_of_list
    ):
        ...
        for class_index in range(self.num_class):
            num_gt_instances = np.sum(
                groundtruth_class_labels[~groundtruth_is_difficult_list & ~groundtruth_is_group_of_list] == class_index
            )
