...
def get_aug_info(init_size, params):
    ...
    for t in params:
        ...
        if t["transform"] == "CenterCrop":
            ...
            x1 = int(round((w - size[0]) / 2.0))
            y1 = int(round((h - size[1]) / 2.0))
            x2 = x1 + size[0]
            y2 = y1 + size[1]
...
class AVA(data.Dataset):
    def __init__(self, root_path, annotation_path, spatial_transform=None, temporal_transform=None):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(frame_indices)):
            ...
            try:
                ...
                raise RuntimeError(f'Caught "{str(e)}" when loading {image_path}')
            clip.append(img)
        ...
        return {"clip": clip, "aug_info": aug_info, "label": target, "video_name": video_name, "mid_time": mid_time}
