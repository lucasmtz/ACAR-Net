...
class BasicNeck(nn.Module):
    def __init__(self, aug_threshold=0.0, bbox_jitter=None, num_classes=60, multi_class=True):
        super().__init__()
    ...
    def forward(self, data):
        ...
        for idx in range(len(data["aug_info"])):
            ...
            sizes_before_padding.append([1.0 / pad_ratio[0], 1.0 / pad_ratio[1]])

            for label in data["labels"][idx]:
                cur_bbox_id += 1
                if self.training and self.bbox_jitter is not None:
                    bbox_list = bbox_jitter(
                        label["bounding_box"], self.bbox_jitter.get("num", 1), self.bbox_jitter.scale
                    )
                else:
