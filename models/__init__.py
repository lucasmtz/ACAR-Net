...
class AVA_model(nn.Module):
    ...
    def forward(self, data, evaluate=False):
        if not evaluate:  # train mode
            ...
            if o_n["num_rois"] == 0:
                return {
                    ...
                    "targets": o_n["targets"],
                    "num_rois": 0,
                    "filenames": o_n["filenames"],
                    "mid_times": o_n["mid_times"],
                    "bboxes": o_n["bboxes"],
                }

            i_h = {
                "features": o_b["features"],
                "rois": o_n["rois"],
        ...
        noaug_info = [{"crop_box": [0.0, 0.0, 1.0, 1.0], "flip": False, "pad_ratio": [1.0, 1.0]}] * len(data["labels"])
        i_n = {
            "aug_info": noaug_info,
            "labels": data["labels"],
            "filenames": data["filenames"],
