...
def main(local_rank, args):
    ...
    if rank == 0:
        utils.mkdir(opt.result_path)
        utils.mkdir(os.path.join(opt.result_path, "tmp"))
        with open(os.path.join(opt.result_path, "opts.json"), "w") as opt_file:
            json.dump(vars(opt), opt_file, indent=2)
        logger = utils.create_logger(os.path.join(opt.result_path, "log.txt"))
        logger.info(f"opt: {pprint.pformat(opt, indent=2)}")
    ...
    utils.random_seed(opt.manual_seed)
    ...
    if rank == 0:
        logger.info(net)
        logger.info(utils.parameters_string(net))

    if not opt.get("evaluate", False):
        train_aug = opt.train.augmentation

        spatial_transform = [
            getattr(spatial_transforms, aug.type)(**aug.get("kwargs", {})) for aug in train_aug.spatial
        ]
        ...
        train_data = ava.AVA(opt.train.root_path, opt.train.annotation_path, spatial_transform, temporal_transform)

        train_sampler = utils.DistributedSampler(train_data, round_down=True)
        ...
        if rank == 0:
            ...
            logger.info(f"train spatial aug: {spatial_transform}")
            logger.info(f"train temporal aug: {temporal_transform}")

            train_logger = utils.Logger(os.path.join(opt.result_path, "train.log"), ["epoch", "loss", "lr"])
        ...
        optimizer = getattr(optim, optim_opt.type)(net.parameters(), lr=sched_opt.base_lr, **optim_opt.kwargs)
    ...
    val_data = ava.AVAmulticrop(opt.val.root_path, opt.val.annotation_path, spatial_transform, temporal_transform)

    val_sampler = utils.DistributedSampler(val_data, round_down=False)
    ...
    if rank == 0:
        ...
        logger.info(f"val spatial aug: {spatial_transform}")
        logger.info(f"val temporal aug: {temporal_transform}")
        ...
        if len(val_log_items) > 1:
            val_logger = utils.Logger(os.path.join(opt.result_path, "val.log"), val_log_items)

    if opt.get("pretrain", None) is not None:
        utils.load_pretrain(opt.pretrain, net)
    ...
    if opt.get("resume_path", None) is not None:
        ...
        if rank == 0:
            logger.info(f"Resumed from checkpoint {opt.resume_path}")

        if not opt.get("evaluate", False):
            optimizer.load_state_dict(checkpoint["optimizer"])
            scheduler.load_state_dict(checkpoint["scheduler"])
            if rank == 0:
                logger.info(f"Also loaded optimizer and scheduler from checkpoint {opt.resume_path}")

    criterion, act_func = getattr(losses, opt.loss.type)(**opt.loss.get("kwargs", {}))

    if opt.get("evaluate", False):  # evaluation mode
        val_epoch(
            begin_epoch - 1, val_loader, net, criterion, act_func, opt, logger, val_logger, rank, world_size, writer
        )
        ...
        for e in range(begin_epoch, opt.train.n_epochs + 1):
            ...
            if e % opt.train.val_freq == 0:
                val_epoch(e, val_loader, net, criterion, act_func, opt, logger, val_logger, rank, world_size, writer)
...
def train_epoch(
    ...
    batch_time = utils.AverageMeter(opt.print_freq)
    data_time = utils.AverageMeter(opt.print_freq)
    losses = utils.AverageMeter(opt.print_freq)
    global_losses = utils.AverageMeter()
        ...
        if (i + 1) % opt.print_freq == 0 and rank == 0:
            ...
            logger.info(
                ...
                "Loss {loss.val:.4f} ({loss.avg:.4f})".format(
                    epoch, i + 1, len(data_loader), batch_time=batch_time, data_time=data_time, loss=losses
                )
            )
    ...
    if rank == 0:
        ...
        epoch_logger.log({"epoch": epoch, "loss": global_losses.avg, "lr": optimizer.param_groups[0]["lr"]})

        logger.info("-" * 100)
        logger.info("Epoch [{}/{}]\t" "Loss {:.4f}".format(epoch, opt.train.n_epochs, global_losses.avg))

        if epoch % opt.train.save_freq == 0:
            save_file_path = os.path.join(opt.result_path, f"ckpt_{epoch}.pth.tar")
            states = {
                "epoch": epoch,
            ...
            logger.info(f"Checkpoint saved to {save_file_path}")

        logger.info("-" * 100)


def val_epoch(epoch, data_loader, model, criterion, act_func, opt, logger, epoch_logger, rank, world_size, writer):
    if rank == 0:
        ...
        if (i + 1) % opt.print_freq == 0 and rank == 0:
            logger.info(
                ...
                "Data {data_time.val:.3f} ({data_time.avg:.3f})\t".format(
                    epoch, i + 1, len(data_loader), batch_time=batch_time, data_time=data_time
                )
            )
    ...
    if rank == 0:
        val_log = {"epoch": epoch}
        val_str = f"Epoch [{epoch}]"

        if calc_loss:
            writer.add_scalar("val/epoch_loss", final_loss, epoch)
            val_log["loss"] = final_loss
            val_str += f"\tLoss {final_loss:.4f}"

        if save_pred:
            result_file = os.path.join(opt.result_path, "predict_epoch%d.csv" % epoch)
            with open(result_file, "w") as of:
                for r in range(world_size):
                    with open(os.path.join(opt.result_path, "tmp", "predict_rank%d.csv" % r)) as f:
                        of.writelines(f.readlines())

            eval_mAP = opt.val.eval_mAP
            metrics = run_evaluation(
                open(eval_mAP.labelmap),
                open(eval_mAP.groundtruth),
                open(result_file),
                open(eval_mAP.exclusions) if eval_mAP.get("exclusions", None) is not None else None,
                logger,
            )
